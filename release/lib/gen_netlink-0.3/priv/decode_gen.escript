...
make_decoder({Name, Attr, Pos, {atom, Next}}) ->
    io_lib:format("decode_~s(_Family, ~w, <<Value:8>>) ->~n    {~s, decode_~s(Value)}",
        [format_name(Name), Pos, Attr, format_name(make_name(Name, Next))]);
make_decoder({Name, Attr, Pos, {hatom16, Next}}) ->
    io_lib:format("decode_~s(_Family, ~w, Value) ->~n    {~s, decode_~s(decode_huint16(Value))}",
        [format_name(Name), Pos, Attr, format_name(make_name(Name, Next))]);
make_decoder({Name, Attr, Pos, {atom32, Next}}) ->
    io_lib:format("decode_~s(_Family, ~w, <<Value:32>>) ->~n    {~s, decode_~s(Value)}",
        [format_name(Name), Pos, Attr, format_name(make_name(Name, Next))]);
...
make_decoder({Name, Attr, Pos, Type})
    ...
    io_lib:format("decode_~s(_Family, ~w, <<Value:~w>>) ->~n    {~s, decode_flag(flag_info_~s(), Value)}",
        [format_name(Name), Pos, flag_len(Type), Attr, format_name(make_name(Name, Attr))]);
make_decoder({Name, Attr, Pos, Type})
    when Type == hflag8; Type == hflag16; Type == hflag32 ->
    io_lib:format(
        "decode_~s(_Family, ~w, <<Value:~w/native-integer>>) ->~n    {~s, decode_flag(flag_info_~s(), Value)}",
        [format_name(Name), Pos, flag_len(Type), Attr, format_name(make_name(Name, Attr))]);
...
make_decoder({Name, Attr, Pos, {nested, Next}}) ->
    io_lib:format("decode_~s(Family, ~w, Value) ->~n    {~p, nl_dec_nla(Family, fun decode_~s/3, Value)}",
        [format_name(Name), Pos, Attr, format_name(make_name(Name, Next))]);
...
make_decoder({Name, Attr, Pos, struct}) ->
    %% built in
    io_lib:format("decode_~s(_Family, ~w, Value) ->~n    decode_~s(~p, Value)",
        [format_name(Name), Pos, format_name(Name), Attr]);

make_decoder({Name, Attr, Pos, Type}) ->
    io_lib:format("decode_~s(Family, ~w, Value) ->~n    {~p, nl_dec_nla(Family, fun decode_~s/3, Value)}",
        [format_name(Name), Pos, Attr, format_name(make_name(Name, Type))]).
...
make_encoder({Name, Attr, Pos, {atom, Next}}) ->
    io_lib:format("encode_~s(_Family, {~w, Value}) ->~n    encode_uint8(~s, encode_~s(Value))",
        [format_name(Name), Attr, format_id(Pos), format_name(make_name(Name, Next))]);
make_encoder({Name, Attr, Pos, {hatom16, Next}}) ->
    io_lib:format("encode_~s(_Family, {~w, Value}) ->~n    encode_huint16(~s, encode_~s(Value))",
        [format_name(Name), Attr, format_id(Pos), format_name(make_name(Name, Next))]);
make_encoder({Name, Attr, Pos, {atom32, Next}}) ->
    io_lib:format("encode_~s(_Family, {~w, Value}) ->~n    encode_uint32(~s, encode_~s(Value))",
        [format_name(Name), Attr, format_id(Pos), format_name(make_name(Name, Next))]);
...
make_encoder({Name, Attr, Pos, Type})
    ...
    io_lib:format("encode_~s(_Family, {~w, Value}) ->~n    encode_~w(~w, encode_flag(flag_info_~s(), Value))",
        [format_name(Name), Attr, flag2int(Type), Pos, format_name(make_name(Name, Attr))]);

make_encoder({Name, Attr, Pos, if_map}) ->
    io_lib:format(
        "encode_~s(_Family, Value)  when is_tuple(Value), element(1, Value) == ~p ->~n    encode_if_map(~w, Value)",
        [format_name(Name), Attr, Pos]);

make_encoder({Name, Attr, Pos, {nested, Next}}) ->
    io_lib:format("encode_~s(Family, {~p, Value}) ->~n    enc_nla(~s, nl_enc_nla(Family, fun encode_~s/2, Value))",
        [format_name(Name), Attr, format_id(Pos), format_name(make_name(Name, Next))]);
...
make_encoder({Name, Attr, Pos, Type})
    ...
    %% built ins
    io_lib:format(
        "encode_~s(_Family, Value)~n  when is_tuple(Value), element(1, Value) == ~p ->~n    encode_~p(~s, Value)",
        [format_name(Name), Attr, Type, format_id(Pos)]);

make_encoder({Name, Attr, Pos, struct}) ->
    %% built in
    io_lib:format(
        "encode_~s(_Family, Value)~n  when is_tuple(Value), element(1, Value) == ~p ->~n    enc_nla(~s, encode_~s(Value))",
        [format_name(Name), Attr, format_id(Pos), format_name(Name)]);
...
make_encoder({Name, Attr, Pos, Type})
    ...
    Type == protocol; Type == mac; Type == addr ->
    %% built ins
    io_lib:format("encode_~s(_Family, {~p, Value}) ->~n    encode_~p(~s, Value)",
        [format_name(Name), Attr, Type, format_id(Pos)]);

make_encoder({Name, Attr, Pos, Type}) ->
    io_lib:format(
        "encode_~s(Family, {~p, Value}) ->~n    enc_nla(~s bor 16#8000, nl_enc_nla(Family, fun encode_~s/2, Value))",
        [format_name(Name), Attr, format_id(Pos), format_name(make_name(Name, Type))]).
...
make_element_encoder_last({Name, _Attr, _Pos, _Type}) ->
    [io_lib:format(
        "encode_~s(_Family, {Type, Value})~n  when is_integer(Type), is_binary(Value) ->~n    enc_nla(Type, Value)",
        [format_name(Name)])].
